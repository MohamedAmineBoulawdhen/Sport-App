...
export default function Signupathlete() {
  ...
  const errorToRegister: any = useSelector(
  ...
  return (
    <div className="root" style={{ padding: 30, minHeight: '75vh' }}>
      <Typography variant="h6" style={{ fontSize: 25, fontWeight: 'bold' }}>
        {' '}
        Sign up
      </Typography>
      {errorToRegister &&
        !errorToRegister.error &&
        errorToRegister?.msg != 'bad credentials' && (
          <Alert variant="danger">{errorToRegister?.msg}</Alert>
        )}
      {showSuccess && (
        <Alert variant="info">You're officially registered!</Alert>
      )}
      <Form
        noValidate
        validated={validated}
      ...
      >
        ...
        <Row className="mb-3">
          ...
          <Form.Group
            as={Col}
            md="6"
            controlId="validationCustomPasswordConfirmation"
          >
            ...
            <InputGroup hasValidation>
              ...
              />
              <Form.Control.Feedback type="invalid">
                Please confirm your password. It should be at least 8 characters
                long.
              </Form.Control.Feedback>
